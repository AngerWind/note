...
进入https://github.com/alibaba/spring-cloud-alibaba/wiki/%E7%89%88%E6%9C%AC%E8%AF%B4%E6%98%8E查看当前spring cloud alibaba对应的sentinal版本

进入https://github.com/alibaba/Sentinel/releases下载相应的dashboard的jar包
...
```sh
# sentinel.dashboard.auth.password和sentinel.dashboard.auth.username指定控制台账号密码
# csp.sentinel.dashboard.server=localhost:8888表示对当前的sentinal本身也进行流量控制
# server.port=8888指定的是sentinel的网页端口, 默认值为8080
...
spring:
  application:
    name: sentinel-circuitbreaking-8082 # 设置当前实例所属的service的名称
  cloud:
    sentinel:
      transport:
        # sentinel api端口, 不需要修改, 默认就是8719
        port: 8719
        # sentinel 网页端口
        dashboard: localhost:8888
      # 开启饥饿加载, 默认为false, 需要调用一次controller后, 服务才会出现在sentinal控制台中
      # 开启之后, 不需要调用, sentinal控制台中会立即出现当前服务
      eager: true
```
