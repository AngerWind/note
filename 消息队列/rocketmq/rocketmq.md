...
- NameServer: Broker会和每一个NameServer建立连接, 并向NameServer发送心跳和Topic的信息, NameServer是一个无状态节点，节点之间无任何信息同步, 要实现高可用只需要多部署几个就好了。

- Broker:  

  - Broker可以有多台, 他们通过brokerClusterName来区分, brokerClusterName相同的是一个集群
  - 同时Broker又分为Master与Slave，一个Master可以对应多个Slave，但是一个Slave只能对应一个Master，不同的master/slaver通过BrokerName来区分, 相同brokerName的broker是主从关系
  - 相同brokerName的broker之间, brokerID为0的是master, 非0的是slave
  - master可读可写, slave从master上同步数据, slave只能读
...
| Keys           | null   | 选填   | 代表这条消息的业务关键词, **rocketmq会根据keys来建立索引, 后续可根据keys来查询该消息** |
...
- 同步还是异步
- 是否超时
- 通过MessageQueue或者MessageQueueSelector来指定要发送的
...
~~~java
    ...
    void send(final Collection<Message> msgs, final SendCallback sendCallback, final long timeout)
