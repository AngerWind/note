...
3. 将uv的安装目录,  以及`UV_PYTHON_BIN_DIR`和`UV_TOOL_BIN_DIR`环境变量添加到path下
...
如果你的脚本有依赖的库, 比如如下代码
   ...
   > 如果你在一个python项目中使用uv run --with, 那么会自动使用这个项目的依赖

> 你可以使用uv cache clean 来生成这个虚拟环境
...
当然你也可以使用uv来创建一个script, uv会将依赖和python版本内联到这个脚本中
...
然后你可以通过如下的命令添加依赖到这个脚本中

~~~shell
uv add --script example.py 'requests<3' 'rich'
Resolved 9 packages in 1.17s
...
之后你可以使用uv run命令来执行脚本, 不需要指定--with选项了,  uv会帮你创建一个带有所有依赖和python版本的一次性虚拟环境, 然后运行脚本



或者对于已经有的python, 你也可以手动添加需要内联的元数据
...
添加 shebang 可以使脚本无需使用 `uv run` 即可执行——这样就可以轻松运行位于 `PATH` 或当前文件夹中的脚本。

例如，创建一个名为 `greet` 文件，内容如下
...
请确保您的脚本具有可执行权限，例如，使用 `chmod +x greet` ，然后运行脚本：
...
你可以通过如下的命令来锁定一个脚本的依赖版本

~~~shell
uv lock --script example.py
~~~

上面的命令会在脚本的目录下生成一个`example.py.lock`文件, 里面记录了对应的依赖版本

一旦锁定，后续操作如 `uv run --script` 、 `uv add --script` 、 `uv export --script` 和 `uv tree --script` 将重用锁定的依赖项，并在必要时更新锁定文件。



### 使用工具

许多 Python 包都提供了可作为工具使用的应用程序。uv 包专门支持轻松调用和安装这些工具。
...
~~~shell
uvx ruff # 创建一个虚拟环境, 使用第一个找到的python版本, 安装ruff的最新版本, 并执行ruff命令
~~~

可以这么使用的前提是, 你的包名和命令的名字是一样的, 所以可以这样使用



#### 命令与包名不一样

如果你的包名和你运行的命令不一样的话, 那么你需要使用`--from`来指定包名, 后面指定要运行的命令

~~~shell
uvx --from 'ruff==0.3.0' ruff check # 下载ruff@0.3.0 并执行 ruff check
uvx --from 'ruff>0.2.0,<0.3.0' ruff check
uvx --from httpie http # 创建一个虚拟环境, 下载httpie, 并执行http命令
~~~
...
在 Python 工具中，许多库和工具（例如 `mypy`）都提供了可选的**扩展功能**，你可以通过这种方式来启用它们。具体来说：

`mypy[faster-cache,reports]`中的` [faster-cache,reports]`就是传递给` mypy`的一组额外的功能（extras）。

- **`faster-cache`**：启用这个扩展会加速缓存机制，从而提升性能，特别是在多次运行类型检查时，它减少了重复计算的时间。
- **`reports`**：启用这个扩展使得 `mypy` 可以生成报告文件，支持多种格式（例如 HTML 或 XML），以便你能够更清晰地查看类型检查的结果。
...
你也可以使用`--from`选项来指定不同依赖的不同来源

~~~shell
uvx --from git+https://github.com/httpie/cli httpie  # 指定从github拉取依赖
uvx --from git+https://github.com/httpie/cli@master httpie # 指定特定的分支
uvx --from git+https://github.com/httpie/cli@3.2.4 httpie # 指定特定的tag
uvx --from git+https://github.com/httpie/cli@2843b87 httpie # 指定特定的commit
~~~
...
通过uvx创建虚拟环境的时候, 除了安装需要运行的包, 你可以可以安装一些额外的依赖, 通过如下命令

~~~shell
uvx --with mkdocs-material mkdocs --help
~~~

上面命令会同时安装mkdocs和mkdocs-material 两个依赖包到虚拟环境中, 然后执行`mkdocs --help`



#### 安装工具

当然如果你经常使用某个工具, 那么最好将他们直接安装到一个持久化的虚拟环境中, 而不是每次都启动虚拟环境来运行
...
上面的命令会将ruff安装到`${UV_TOOL_DIR}`目录下面, 并将ruff的可执行二进制文件放在`${UV_TOOL_BIN_DIR}`目录下面

> uv还会判断`${UV_TOOL_BIN_DIR}`目录是不是在你的path下, 如果不是, 会提示你执行如下代码来将该目录添加到path下

> 如果你安装了ruff, 又安装了httpie, 那么他会在执行的时候会在同一个虚拟环境中, 这个虚拟环境专门用来执行工具的



之后你就可以直接使用`ruff --version `来执行命令了, **并且如果你是在项目中使用ruff的话, 那么他也不会被作为项目的依赖, 可以有效的隔离工具和项目依赖**
...
在执行upgrade的时候, 只会将版本更新到最新的小版本, 例如： 先 `uv tool install ruff >=0.3,<0.4` 然后执行 `uv tool upgrade ruff` 即可将 Ruff 升级到 `>=0.3,<0.4` 范围内的最新版本。
...
你在使用`uvx, uv tool run, uv tool install, uv tool upgrade`的时候,  创建的虚拟环境将使用第一个找到的python解释器,  您可以使用 `--python` 参数指定要使用的 Python 解释器。
...
6. 列出永久虚拟环境中的工具安装在哪里
   ...
   ~~~shell
   uv tool dir
   D:\sdk\uv\uv_tool_dir
   ~~~

   你也可以使用`UV_TOOL_DIR`环境变量来自定义位置
...
2. 运行脚本
   ...
   > 在项目中运行脚本,  这个脚本会自动使用当前项目中的所有依赖, 不需要通过uv run --with来添加依赖了
