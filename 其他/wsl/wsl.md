...
> 在最新的22h2版本的windows中, 你可以直接使用wsl --install 来一键开启wsl和虚拟机功能
...
在运行linux发行版本的时候,  你可以指定使用wsl1还是wsl2来运行发行版本, 当然你如果不指定的话, 那么也会有一个默认使用的wsl版本来运行

你可以通过如下的命令来查看发行版本使用的是wsl1还是wsl2来运行的
...
上面可以看出, 两个linux发行版本都是使用wsl2来运行的



你可以通过如下命令来查看默认使用的wsl版本
...
你可以通过如下的命令来设置默认的wsl版本,  以及设置不同的发行版本使用的wsl的版本

~~~shell
wsl --set-default-version 2 # 设置默认使用的wsl的版本
wsl --set-version Ubuntu-22.04 2 # 设置Ubuntu-22.04使用wsl2来运行
~~~
...
可以看到上面默认使用wsl2来运行Ubuntu, 当然你也可以使用如下命令来进行修改
...
wsl --manage Ubuntu-24.04 --set-default-user root
~~~

> 有时候上面的命令会不起作用, 那么你可以直接进入到ubuntu中, 然后vim /etc/wsl.conf
>
> 修改如下的内容, 然后重启ubuntu
>
> ~~~shell
...
上面的使用最常用的就是用来修改root的密码了, 因为新安装的wsl的root的密码并不知道

~~~shell
wsl -d Ubuntu -u root -e passwd
~~~
