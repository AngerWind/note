...
Cache Aside Pattern (写直达法)：缓存调用者在更新完数据库后再去更新缓存，也称之为**双写方案**

Read/Write Through Pattern : 缓存和数据库整合为一个服务, 由服务来维护一致性, 调用者调用该服务即可, 无需关系一致性 

Write Behind Caching Pattern(写回法) ：调用者只操作缓存，由一个独立的线程定时将变化的数据同步到数据库中，实现最终一致

![1653322857620](.\Redis实战篇.assets\1653322857620.png)

#### 2.3.2 、数据库和缓存不一致采用什么方案

对于方案3, 因为变化的数据都在内存中,有可能会丢数据, 而且在一个同步周期内, 多次操作, 此时缓存和数据库是不一致的.

**综合考虑使用方案一，虽然需要进行编码, 但是可控性高**
...
1. 删除缓存还是更新缓存
   ...
   * 更新缓存：每次更新数据库都更新缓存, 当中间如果没有人查询，那么这个更新动作实际上只有最后一次生效，中间的更新动作意义并不大
   * 删除缓存：更新数据库时让缓存失效，查询时再更新缓存, **有效更新更多**
...
2. 先操作缓存还是数据库
   ...
   ![1653323595206](img/2-Redis实战篇/1653323595206.png)

3. 如何保证数据库和缓存的操作同时成功和失败
...
**我们把过期时间设置缓存的value中, 而不是直接设置缓存的过期时间**

![image-20240522164403338](img/2-Redis实战篇/image-20240522164403338.png)

假设线程1去查询缓存，他先要获取过期时间来判断缓存是否过期, 如果过期了，此时线程1去获取分布式锁，去进行 以前的重构数据的逻辑，然后才释放锁

假设现在线程2过来访问，由于线程1持有着锁，所以线程2无法获得锁, 他可以有两种方式: 

- 那么他就直接返回过期数据, 有点在于性能高, 缺点在于重建缓存之前都是脏数据
- sleep一段时间, 然后使用double check去检查, 直到线程1重构完缓存, 这种方式和互斥锁没什么不同, 还更麻烦
...
比如时间是否充足，如果时间充足，则进一步判断库存是否足够，如果两者都满足，则创建订单, 扣减库存, 然后返回订单id，如果有一个条件不满足则直接结束。
...
**存在问题：**现在的问题还是和之前一样，并发过来，查询数据库，都不存在订单，**<font color=red>所以我们还是需要加锁，但是乐观锁比较适合更新数据，而现在是插入数据，所以我们需要使用悲观锁操作</font>**
...
* 释放锁：
  ...
  * 手动释放
  ...
  ![1653382669900](img/2-Redis实战篇/1653382669900.png)
...
答案是课程中的好，因为如果你采用我刚说的方式，如果访问的人很多，那么线程池中的线程可能一下子就被消耗完了，而且你使用上述方案，最大的特点在于，你觉得时效性会非常重要，但是你想想是吗？**<font color=red>并不是，比如我只要确定他能做这件事，然后我后边慢慢做就可以了，我并不需要他一口气做完这件事</font>**，所以我们应当采用的是课程中，类似消息队列的方式来完成我们的需求，而不是使用线程池或者是异步编排的方式来完成这个需求
...
* 无法避免消息丢失, 即消费者pop消息之后, 还没有处理就挂掉了, 那么消息就丢失了
* 只支持单消费者, pop之后消息就会没有



### 7.2 Redis消息队列-基于PubSub的消息队列

PubSub（发布订阅）是Redis2.0版本引入的消息传递模型。消费者可以订阅一个或多个channel，生产者向对应channel发送消息后，所有订阅者都能收到相关消息。

常用命令如下: 

1.  `SUBSCRIBE channel1 channel2....` ：订阅一个或多个频道, 不需要创建, 直接订阅即可

2.  `PUBLISH channel1 msg` ：向一个频道发送消息

3. `PSUBSCRIBE pattern [pattern]` ：订阅与pattern格式匹配的所有频道, pattern可以使用通配符

   ~~~bash
   psubscribe pattern h?llo # ?匹配1个字符
   psubscribe pattern h*llo # *匹配零个或者多个字符
   psubscribe pattern h[ae]llo # [ae]表示匹配a或者e
   ~~~
...
* 消息会直接发送到consumer, 而不会持久化到redis中, 有人收就收, 没人收就丢了
* 消费者没有ack机制, 无法避免消息没有处理
* consumer接受到消息之后, 会缓存起来, 而缓存有上限, 超出时数据丢失



### 7.3 Redis消息队列-基于Stream的消息队列
...
1. `xadd key [nomkstream] [maxlen | minid [= | ~] threshold [ limit count ] ] * | id field value [field value ] `
   ...
   - nomkstream: stream不存在的时候不创建, 默认是创建的

   - maxlen: 限制流的长度, 超过时旧消息被删除
   - minid: 移除流中小于给定id的条目
   - = : 严格的最大长度或最小 ID，即超过/小于这个值时才会触发修剪操作
   - ~: 近似的最大长度或最小 ID，提供一个更宽松的修剪条件以**提高性能**。
   - threshold: 最大长度或最小 ID 的阈值。
   - limit count: 指定修剪时最多删除多少个条目，用于限制修剪操作的影响范围。

   - \* | id:  设置消息的id, *表示redis自动生成, id表示自己指定, id要符合格式: `事件戳-递增数字`, 比如: `1644804662702-0`

   - field value: 一条消息中的一个entry, 一条消息可以有多个entry

   ~~~bash
   # 向users的stream发送一条消息, 内容是{name=jack, age=21}, id由redis自动生成
   # 命令执行后会返回消息的id
   xadd users * name jack age 21 
   
   # 自定义id
   XADD mystream 1526985051234-0 field1 value1 field2 value2
   # 精确限制stream的长度为1000, 并且修剪操作在一次删除时最多删除 100 个条目
   XADD mystream MAXLEN = 1000 LIMIT 100 * field1 value1 field2 value2
   # 近似限制流的最大长度为1000, 提升性能, 可能会保留略多于 1000 条记录。
   XADD mystream MAXLEN ~ 1000 * field1 value1 field2 value2
   ~~~

   ![1653577349691](img/2-Redis实战篇/1653577349691.png)

2. `xread [count num] [block milliseconds] streams key [key ...] ID [ID ...]`

   - count num: 本次读取消息的最大数量
   - block milliseconds: 当没有消息可读时, 阻塞的时间,   不指定时立即返回, **block 0表示永久阻塞**
   - key: 从哪个stream中读取消息, 可以指定多个
   - id: 只返回大于该id的消息,  **0表示从第一条消息开始返回, $表示只读取最新接受的消息**

   ~~~bash
   # 从users的stream中读取一条消息, 从开始读
   xread count 1 stream users 0
   # 读取最新的消息, 如果没有消息就block 5ms
   xread count 1 block 5 stream users $ 
   ~~~

   ![1653577643629](img/2-Redis实战篇/1653577643629.png)
...
### 7.4 Redis消息队列-基于Stream的消息队列-消费者组

消费者组（Consumer Group）：将多个消费者划分到一个组中，监听同一个队列。具备下列特点：

1. 消息分流: 队列中的消息会分流给组内的不同消费者, 从而加快消息的处理速度
2. 消息标识: redis会维护一个标识, 记录消费者组最后一个被处理的消息, 哪怕消费者宕机重启, 还是会从标识之后读取消息, 保证每个消息都会被消费
3. 消息确认: 消费者获取消息之后, 消息处于pending状态, 并存入pending list. 当消息处理完后需要通过ack来确认消息, 标记消息已处理, 才会从pending list中移除

常见命令如下: 

1. `xgroup create key groupName id [mkstream]`

   给指定的stream创建消费者组, key表示stream, 

   id表示group的初始标识, 0标识当前消费者组从第一条消息开始读, $表示消费者组从最新的数据开始读

   mkstream:  stream不存在时自动创建, 默认不自动创建, 此时如果没有stream, 那么group就会创建失败

2. `XGROUP DESTORY key groupName`
   
   删除指定的stream的group
   
3. `XGROUP CREATECONSUMER key groupname consumername`

    给指定的消费者组添加消费者

4. `XGROUP DELCONSUMER key groupname consumername`
...
5. 从消费者组读取消息
   ...
   * BLOCK milliseconds：当没有消息时最长等待时间, block 0表示永久等待, 不指定就立即返回
   ...
   * ID：获取消息的起始ID,  
     ...
     - **>表示从下一个未消费的消息开始**
       ...
       正常情况下我们每次读取都要传入>, 保证消息全部被读取

     - ID: 根据指定的id从pending list中获取已经获取但是没有ack的消息.

       用于容错, 我们每次读取都应该传0, 即消费pending list中的第一条消息, 这样我们处理并ack后, 那么下一次读取穿0, 又是第一条消息
...
### 7.5 基于Redis的Stream结构作为消息队列，实现异步秒杀下单
...
GEO是可以存储经纬度的一个集合, 他的底层实现就是ZSet,  GEO会将member转换为zset中的member, 然后将经纬度根据特定的算法转换为score

所以在通过GUI客户端查看GEO时, 显示的类型有可能会是zset

![image-20240521145759194](img/2-Redis实战篇/image-20240521145759194.png)

常见的命令有: 

* `GEOADD key longitude latitude member`：往集合中添加一个经纬度坐标, longitude经度, latitudu维度, member成员, 可以是地名

* `GEOPOS key member`:   返回member的经纬度

* `GEOHASH key member`：将指定member的坐标转为hash字符串形式并返回

* `GEODIST  key member1 member2  [m|km|ft|mi]`：计算member1和member2之间的距离, m, km, ft, mi都是距离的单位

* `GEORADIUS key longitude latitude radius m|km|ft|mi  [withdist] [withhash] [asc | desc] [count  num]`：longitude和latitude作为圆心, radius作为半径,  找到key中所有在圆内的member,  在6.2之后废弃

  m,km,ft, mi表示半径的单位

  withdist表示结果带上他们的距离

  withdist结果带上member的坐标hash值

  count 表示只返回前n个

  asc|desc表示排序方式

* `GEOSEARCH key [frommember member] [fromlonlat  longitude latitude ] [byradius radius m | km | ft | mi] [bybox width height  m | km | ft | mi ] [ asc | desc ] [count num ] [withdist ] [withhash]`：

  返回指定key中的指定形状范围内的所有member, 6.2后的新功能

  frommember和fromlonlat:  指定member作为中心点, 还是经纬度作为中心点

  byradius和bybox: 指定形状是圆形还是矩形
...
用户一次签到，就是一条记录，假如有1000万用户，平均每人每年签到次数为10次，则这张表一年的数据量为 1亿条, 每签到一次需要使用（8 + 8 + 1 + 1 + 3 + 1）共22 字节的内存, 那么一个月最多需要660字节的磁盘空间



如果我们使用redis, 那么我们可以使用Redis中的bitmap数据结构, 将每个用户id和每个月结合, 每个用户每个月的天道记录放在一个bitmap中, 这样一个月的记录只需要32位, 即4字节, 如果用户签到了, 那么就将该天对应的bit设置为1
...
* `GETBIT key offset` ：获取指定位置（offset）的bit值

* `BITCOUNT key` ：统计BitMap中值为1的bit位的数量

* `BITFIELD key [get type offset] [set type offset value ] [incrby type offset increment] ` 

  get type offset :  从offset开始, 获取指定位数的值, 比如

  ~~~bash
  bitfield bm1 get u2 0 # 从0开始, 获取两个bit位, 并转换为无符号整型
  bitfield bm1 get i3 1 # 从1开始, 获取3个bit位, 转为为有符号整型
  ~~~

  set type offset value: 从offset开始, 设置指定位数的值

  ~~~bash
  bitfield bm1 set u2 0 3 # 从0开始, 将3转换为无符号整型11, 然后设置到第0, 1的bit位
  ~~~

* `BITFIELD_RO key ` ：获取BitMap中bit数组，并以十进制形式返回

* `BITOP and| or | xor | not  destkey key1 key2 ...`  ：将多个BitMap的结果做位运算（与 、或、异或, 非), 然后将结果存储到destkey中

* `BITPOS key 0|1 start end` ：查找bit数组中指定范围内第一个0或1出现的位置

  ~~~bash
  bitpos bm1 1 0 3 # 从[0-3)查询第一次出现1的位置
  ~~~
...
1. `pfadd key element1 element2...` 向hll中添加元素
2. `pfcount key`  返回hll中元素的个数, 有一定的误差, 重复的元素不做计数
3. `pfmerge destkey sourcekey1 sourcekey2` 合并多个hll到一个hll中
