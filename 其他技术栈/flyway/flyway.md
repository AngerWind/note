...
repeatable migration脚本可以结合占位符来使用,  你可以在springboot中定义占位符, 然后再migrations脚本中使用, 这样flyway在执行脚本之前, 会先将占位符替换掉, 然后执行

~~~sql
-- 定义了一个占位符, 他将环境变量BUILD_TS作为值, 如果没有的话默认使用dev
spring.flyway.placeholders.build_timestamp=${BUILD_TS:dev}
~~~

然后你可以在repeatable migration的顶部添加如下的一行代码

~~~sql
-- ${build_timestamp}
~~~

这样你每次启动springboot的时候, 因为BUILD_TS的时间的不同, 那么这个repeatable migration的check sum就会发生变化, 从而自动的执行这个migration
...
baseline是指一个基线版本, 即数据库在某一时刻的快照, 他的作用是可以让我们直接重建数据库的状态, 而不需要根据versioned migrations一步一步来重建数据库
...
比如我们有一个`B5__snapshot_after_V5.sql`, 那么这个文件中保存的就是数据库在v5这个版本的schema的快照



flyway在执行的时候, 如果发现这个数据库中还没有`flyway_history`表, 那么就会认为这是一个新的数据库, 那么他会直接执行baseline文件来快速重建, 而忽略低于该baseline的versioned migration

比如如果flyway发现migration目录下面有一个`B5__snapshot_after_V5.sql`文件, 那么他会直接执行这个脚本来重建到v5版本, 如果还有更高版本的versioned migration, 那么也会执行



如果flyway在执行的时候, 发现数据库中已经有了`flyway_history`表,  那么他会认为这个数据库已经执行过migration了, 那么他会老老实实的按照versioned migration脚本来重建数据库



#### 有条件的执行migration

有三种比较简单的方式来执行migration

1. 直接使用sql中的create if not exists

   ~~~sql
   CREATE INDEX IF NOT EXISTS idx_email ON civilian(email);
   ~~~

2. 如果更复杂的条件, 那么你可以使用匿名代码块来控制
...
3. 第三种方式是占位符和Do模块结合
   ...
   首先我们要在springboot的application.properties中添加一个占位符

   ~~~sql
   spring.flyway.placeholders.seed_demo_data=true
   ~~~

   然后再migrations脚本中使用这个占位符
...
之后你需要再application.properties文件中添加数据库的jdbc依赖
...
# 下面两个数据的区别在于, 上面的是无关jpa具体实现的, 下面的是指定关闭jpa的hibernate实现的ddl功能
...
1. 首先你需要创建一个springboot的jpa项目, 并在项目中添加一个entity
...
2. 然后你需要通过IDEA的数据库工具来连接到pg

   ![img](img/3-ij-db-connect.gif)



#### 使用插件生成Migrations

之后你可以通过插件, 根据jpa的实体类来生成对应的migrations脚本
...
之后我们可以使用插件来生成对应的更改的Migrations脚本, 也就是idea插件中的`Flyway Migration`, 这种就是使用当前的entity和数据中的表进行对比, 生成对应的Migration脚本
...
![img](img/7-generate-2nd-flyway-migration.gif)





#### 手动编写Migrations并修改JPA实体

我们还可以手动编写migrations脚本, 然后通过插件根据migrations脚本自动帮助我们修改jpa类

首先我们创建一个migrations

~~~sql
-- V3__add_published_at_col_to_bookmarks.sql
ALTER TABLE bookmarks ADD published_at timestamp;
~~~

重新启动springboot, 就会应用这个脚本, 将`published_at`这个字段添加到表中

之后我们需要将数据库中的表结构同步到jpa实体类中, 我们可以在*数据库*工具窗口中，右键点击`书签`表，选择 *“从数据库创建实体属性* ”。“ *从数据库获取实体属性* ”对话框将显示尚未添加到`书签`实体的列。选择相关列并点击*确定* 

![img](https://blog.jetbrains.com/wp-content/uploads/2024/11/8-sync-entity-from-table.gif)
...
### flyway和github actions结合

https://bell-sw.com/blog/how-to-use-flyway-with-spring-boot/

使用这种方式, 只要你推送代码到github上面, 那么就会触发github actions, 然后再actions中就会去执行`mvn -B flyway:migrate` 这个命令, 连接到你的数据库上, 执行migration脚本
