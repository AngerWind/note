...
----
<1> 将特质 FlyingAbility 添加到 Bird 类功能中
<2> 实例化一个新的 Bird
<3> Bird 类自动获取 FlyingAbility 特质的行为
...
----
<1> 在特质中定义私有方法 greetingMessage
<2> 公共 greet 消息默认调用 greetingMessage
<3> 创建一个实现该特质的类
<4> greet 可以被调用
<5> 但 greetingMessage 不行

WARNING: 特质仅支持 public 和 private 方法。 protected 和 `package private` 范围均不受支持。
...
----
<1> 普通接口的声明
<2> 将 Named 添加到已实现的接口列表中
<3> 声明一个实现 Greetable 特质的类
<4> 实现缺少的 name 方法
<5> greeting 实现来自特质
<6> 确保 Person 实现 Named 接口
<7> 确保 Person 实现 Greetable 特质
...
----
<1> 在特质中声明私有字段 count
<2> 声明一个公共方法 count 来增加计数器并返回它
<3> 声明一个实现 Counter 特质的类
<4> count 方法可以使用私有字段来保存状态



TIP: 这是与 Java 8 http://docs.oracle.com/javase/tutorial/java/IandI/defaultmethods.html[Java 8 virtual extension methods]的一个主要区别。虽然虚拟扩展方法不携带状态，但特质可以。此外，从 Java 6 开始支持 Groovy 中的特质，因为它们的实现不依赖于虚拟扩展方法。这意味着即使一个特质可以从 Java 的角度视为常规接口，该接口也不会具有default方法，只有抽象方法。
...
WARNING: 虽然特质支持公共字段，但不建议使用它们，并被认为是一种不好的做法。
...
----
<1> Duck 类同时实现 FlyingAbility 和 SpeakingAbility
<2> 创建 Duck 的新实例
<3> 我们可以从 FlyingAbility 调用方法 fly
<4> 还有 SpeakingAbility 中的方法 speak
...
<3> 鸭子仍然在飞
<4> quack 来自 Duck 类
<5> speak 不再使用 SpeakingAbility 中的默认实现
...
-----
<1> Named 特质定义单个 name 属性
<2> Polite 特性扩展了 Named 特性
<3> Polite 添加了一个新方法，可以访问父特质的 name 属性
...
----
<1> WithId 特质定义 id 属性
<2> WithName 特质定义 name 属性
<3> Identified 是继承 WithId 和 WithName 的特质
...
<2> Duck 类 实现了 methodMissing  方法方法
...
<8> 调用 Dynamic 上的现有方法
<9> 但由于 methodMissing 特性而调用不存在的方法
...
----
<1> Extra 特质定义了 extra 方法
<2> Something 类未实现 Extra 特质
...
----
<1> 使用 as 关键字在运行时将对象强制为特质
<2> 然后可以在对象上调用 extra
<3> 并且 doSomething 仍然可以调用

IMPORTANT: 当将对象强制为特质时，操作的结果与原来的实例不是同一个实例。被强制的对象将实现原始对象实现的特质和接口，但不会是原始类的实例。
...
----
<1> 调用 methodFromA 将失败，因为 C 未实现 A
<2> 调用 methodFromB 将失败，因为 C 未实现 B
<3> withTrait 将把 c 包装成实现 A 和 B 的东西
<4> methodFromA 现在将通过，因为 d 实现了 A
<5> methodFromB 现在将通过，因为 d 也实现了 B

IMPORTANT: 当将一个对象强制为多个特质时，操作的结果与原来的实例不是同一个实例。被强制的对象将实现原始对象实现的特质和接口，但不会是原始类的实例。
...
<1> 显式实现 on 方法
<2> 执行日志记录
<3> 继续执行 DefaultHandler 特质中的行为

这是可行的，但这种方法有缺点：

. 日志逻辑绑定到“具体”处理程序
. 我们在 on 方法中显式引用了 DefaultHandler ，这意味着如果我们碰巧更改了类实现的特质，代码将会被破坏
...
. 如果该类实现了另一个特质，则调用将委托给链中的下一个特质
. 如果链中没有剩余的特质了，则 super 引用实现类的超类（this）
...
----
<1> 定义一个名为 Filtering 的特质，应该在运行时应用于 StringBuilder
<2> 重新定义 append 方法
<3> 删除字符串中的所有“o”
<4> 然后委托给 super
<5> 如果调用 toString ，则委托给 super.toString
<6> StringBuilder 实例上 Filtering 特质的运行时实现
<7> 已附加的字符串不再包含字母 o
...
----
<1> greet 方法不是抽象方法，调用抽象方法 getName
<2> getName 是一个抽象方法
...
WARNING: 以下说明需谨慎。静态成员支持正在进行中，并且仍处于试验阶段。以下信息仅适用于 {groovyVersion}。

可以在特质中定义静态方法，但它有许多限制：
...
通常不应混合具有相同签名的静态方法和实例方法。应用特质的正常规则适用（包括多重继承冲突解决）。如果选择的方法是静态的，但某些实现的特质具有实例变体，则会发生编译错误。如果选择的方法是实例变体，则静态变体将被忽略（在这种情况下，其行为类似于 Java 接口中的静态方法）。
...
<5> 但 Baz 上的静态字段 CALLED 不是，因为它是不同的
...
CAUTION: Traits 与 AST 转换不是正式兼容。其中一些（例如 @CompileStatic ）将应用于特质本身（而不是实现类），而其他一些将应用于实现类和特质。绝对不能保证 AST 转换会像在常规类上一样在特质上运行，因此使用它需要您自担风险！
